...
export default {
    ...
    endSurvey() {
      ...
      db.get(this.user._id)
        .then(doc => {
          ...
          let tempFind = false;
          console.log("doc.surveys", doc.surveys);
          for (let index = 0; index < doc.surveys.length; index++) {
            if (this.questionnaire.id === doc.surveys[index]._id) {
              tempFind = true;
            }
          }
          if (!tempFind) {
            this.user.surveys.push(this.questionnaire);
            this.user._rev = doc._rev;

            db.put(this.user, options)
              .then(function(response) {
                console.log("Insert survey success response", response);
              })
              .catch(function(err) {
      ...
      router.push({
        ...
        query: {
          ...
          idSurvey: this.questionnaire.id
        }
      });
    },
    currentSurvey() {
  ...
  data() {
    return {
      ...
      questionnaire: {
        id: "BadassSurveyUpgrade__app", //un questionnaire
